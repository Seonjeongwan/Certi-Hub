...
export default function CertModal({
  ...
  return (
    <div
      className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[2000] flex items-end sm:items-center justify-center sm:p-6"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="bg-white rounded-t-2xl sm:rounded-2xl max-w-[600px] w-full max-h-[90vh] sm:max-h-[80vh] overflow-y-auto p-5 sm:p-8 relative">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-transparent border-none text-xl text-[#858a8d] cursor-pointer w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-gray-100 hover:text-[#1b1c1d]"
        >
        ...
        {/* Tag */}
        <span
          className="inline-block px-3 py-1 rounded-md text-xs font-bold mb-3 sm:mb-4"
        ...
        <h2 className="text-[20px] sm:text-[22px] font-bold mb-1 sm:mb-2 pr-8">{cert.name_ko}</h2>
        <p className="text-[#858a8d] text-sm mb-1">{cert.name_en}</p>

        {/* Info Grid */}
        <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-4 sm:mt-5">
          <div className="bg-gray-50 p-3 sm:p-3.5 rounded-[10px]">
            <div className="text-[11px] sm:text-xs text-[#858a8d] font-semibold mb-1">
              레벨
            </div>
            <div className="text-[14px] sm:text-[15px] font-bold">
              <span
                className={`px-2 py-0.5 rounded text-[12px] sm:text-[13px] ${levelColors[levelClass]}`}
              >
          ...
          <div className="bg-gray-50 p-3 sm:p-3.5 rounded-[10px]">
            ...
            <div className="text-[14px] sm:text-[15px] font-bold">{cert.tag}</div>
          </div>
          <div className="bg-gray-50 p-3 sm:p-3.5 rounded-[10px]">
            <div className="text-[11px] sm:text-xs text-[#858a8d] font-semibold mb-1">
              세부 분야
          ...
          <div className="bg-gray-50 p-3 sm:p-3.5 rounded-[10px]">
            ...
            <div className="text-[14px] sm:text-[15px] font-bold">{levelIdx + 1} / 4</div>
          </div>
        </div>
          ...
          <div className="mt-5 sm:mt-6">
            ...
            <div className="space-y-2.5 sm:space-y-3">
              {examRounds.map((rd) => (
                ...
                >
                  {/* 회차 헤더 */}
                  <div className="bg-gray-50 px-3 sm:px-4 py-2 flex items-center gap-2 border-b border-gray-100">
                    <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-primary text-white text-[10px] sm:text-xs font-bold flex items-center justify-center">
                      {rd.round}
                    </span>
                  ...
                  <div className="divide-y divide-gray-50">
                    {/* 접수 기간 */}
                    {rd.reg && (
                      <div className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-50/50">
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-[#93c5fd] flex items-center justify-center flex-shrink-0">
                          <i className="fas fa-file-signature text-white text-[9px] sm:text-[10px]" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] sm:text-[11px] text-blue-600 font-semibold">
                            접수 기간
                          </div>
                          <div className="text-[12px] sm:text-[13px] font-bold text-blue-900">
                    ...
                    {/* 시험일 */}
                    {rd.exam && (
                      <div className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-red-50/50">
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-[#ef4444] flex items-center justify-center flex-shrink-0">
                          <i className="fas fa-pen-to-square text-white text-[9px] sm:text-[10px]" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] sm:text-[11px] text-red-600 font-semibold">
                            시험일
                          </div>
                          <div className="text-[12px] sm:text-[13px] font-bold text-red-900">
                    ...
                    {/* 합격 발표 */}
                    {rd.result && (
                      <div className="flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-green-50/50">
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-[#22c55e] flex items-center justify-center flex-shrink-0">
                          <i className="fas fa-bullhorn text-white text-[9px] sm:text-[10px]" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] sm:text-[11px] text-green-600 font-semibold">
                            합격 발표
                          </div>
                          <div className="text-[12px] sm:text-[13px] font-bold text-green-900">
        ...
        {certEvents.length === 0 && (
          isCBTCert(cert) ? (
            ...
            <div className="mt-5 sm:mt-6 bg-emerald-50 p-4 sm:p-5 rounded-[10px]">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
                  <i className="fas fa-clock text-emerald-600 text-sm" />
                </div>
                <div>
                  <div className="text-[13px] sm:text-sm font-bold text-emerald-800">상시 응시 가능 (CBT)</div>
                  <div className="text-[11px] sm:text-xs text-emerald-600">Computer Based Testing</div>
                </div>
              </div>
              <p className="text-[12px] sm:text-[13px] text-emerald-700 leading-relaxed mt-2">
            ...
            <div className="mt-5 sm:mt-6 bg-violet-50 p-4 sm:p-5 rounded-[10px]">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-violet-100 flex items-center justify-center shrink-0">
                  <i className="fas fa-bullhorn text-violet-600 text-sm" />
                </div>
                <div>
                  <div className="text-[13px] sm:text-sm font-bold text-violet-800">비정기 시험 · 공지 확인 필요</div>
                  <div className="text-[11px] sm:text-xs text-violet-600">시험 일정이 별도 공지됩니다</div>
                </div>
              </div>
              <p className="text-[12px] sm:text-[13px] text-violet-700 leading-relaxed mt-2">
            ...
            <div className="mt-5 sm:mt-6 bg-amber-50 p-4 sm:p-5 rounded-[10px]">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-amber-100 flex items-center justify-center shrink-0">
                  <i className="fas fa-rotate text-amber-600 text-sm" />
                </div>
                <div>
                  <div className="text-[13px] sm:text-sm font-bold text-amber-800">시험 일정 불러오는 중</div>
                  <div className="text-[11px] sm:text-xs text-amber-600">공식 사이트에서 일정을 확인해 주세요</div>
                </div>
              </div>
              <p className="text-[12px] sm:text-[13px] text-amber-700 leading-relaxed mt-2">
        ...
        <div className="mt-4 sm:mt-5">
          {cert.official_url ? (
            <a
              ...
              className="flex items-center justify-center gap-2 w-full py-3 sm:py-3.5 px-6 bg-primary hover:bg-primary/90 text-white font-bold text-[14px] sm:text-[15px] rounded-xl transition-all duration-200 shadow-md hover:shadow-lg hover:-translate-y-0.5 no-underline"
            >
              <i className="fas fa-external-link-alt" />
              공식 사이트 바로가기
            </a>
          ) : (
            <div className="flex items-center justify-center gap-2 w-full py-3 sm:py-3.5 px-6 bg-gray-200 text-gray-500 font-bold text-[14px] sm:text-[15px] rounded-xl cursor-not-allowed">
